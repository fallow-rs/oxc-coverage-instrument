...
- [x] Add a test that opens the output root, replaces a valid nested directory
  entry with a symlink to an outside directory before the nested leaf write,
  and asserts the outside sentinel is unchanged.
- [x] Add a test that hard-links the future report leaf to an outside sentinel,
  writes the report, and asserts the outside inode content is unchanged.
- [x] Run the focused tests and confirm RED because path-based writes either
...
- [x] Add exact dependencies `cap-std = "4.0.2"` and
  `cap-fs-ext = "4.0.2"` behind the existing `html` feature.
- [x] Implement component validation and handle-relative
  `open_dir_nofollow` traversal in `output.rs`.
- [x] Implement temporary `create_new` leaf writes and same-directory rename.
...
- [x] Ensure every failure removes its temporary sibling and never truncates
  an existing destination inode.
- [x] Replace every HTML asset, folder-index, and detail-page `fs::write` and
  `create_dir_all` call with `OutputDir` operations.
- [x] Remove the obsolete check-then-use symlink preflight.
...
- [x] Run the focused RED tests and confirm GREEN.
- [x] Run report crate tests, strict clippy, and formatting.
- [x] Build the N-API binding, run the real Vitest example, and render its
