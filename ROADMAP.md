...
- **`ci.yml`**: cross-platform `check` matrix (ubuntu / macos / windows), `msrv` (1.95), `doc` (`RUSTDOCFLAGS=-D warnings`), `typos`, `audit` (cargo-audit / RustSec), `deny` (cargo-deny per `deny.toml`), `shear` (unused / misplaced deps), `zizmor` (workflow security), `napi` (build + `node test.mjs`), `istanbul-diff` (byte parity against `istanbul-lib-instrument` on 27 fixtures), `vitest-example-binding` + `vitest-typescript-example` (end-to-end Vitest adapter smoke, built once then run across a Node 22/24 x TypeScript 6/7 matrix), and a single `ci-ok` aggregator job for branch protection
