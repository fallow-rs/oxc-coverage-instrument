//! Parses the emitted LCOV back with the upstream `lcov` crate to assert the
//! tracefile is structurally valid, so drift in the hand-rolled emitter (a
//! missing `end_of_record`, wrong record order, malformed `BRDA` arity) fails
//! here instead of at Codecov / GitLab / `genhtml` parse time.
//!
//! The `lcov` crate is a dev-dependency only; the emitter itself takes no
...
fn emitted_lcov_parses_with_upstream_crate() {
    ...
    for record in reader {
        let _record = record.expect("lcov parser must accept the emitted tracefile");
        record_count += 1;
    }
