//! End-to-end smoke for the HTML reporter against a real source file on
//! disk, the shape a `coverage-final.json` has in practice. The unit tests
//! in `src/html/mod.rs` cover edge cases on synthetic input.
//!
...
fn renders_actual_source_with_coverage_classes() {
    ...
    // The coverage key is relative and resolved against `root_dir`; an
    // absolute Windows path (`C:\\...`) would propagate through the report
    // tree as a non-relative folder component, which Windows path APIs
    // reject.
