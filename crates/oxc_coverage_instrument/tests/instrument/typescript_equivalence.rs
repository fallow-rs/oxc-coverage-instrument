...
fn ts_direct_emits_executable_js_without_type_annotations() {
    // The line-set equivalence tests above pass whether or not the strip pass
    // ran, because `Span` values survive it either way. This one asserts on the
    // emitted code string instead.
    let ts_opts = InstrumentOptions { strip_typescript: true, ..InstrumentOptions::default() };
