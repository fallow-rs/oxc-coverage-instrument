//! Benchmarks for `apply_v8_coverage` and the source-map trailer helpers.
...
/// Return shape of `branch_heavy_fixture`: generated source, the coverage map,
/// the V8 function ranges, and the per-branch span table.
type BranchFixture =
...
fn line_fixture(lines: u32, unicode: bool) -> (String, FileCoverage, Vec<V8FunctionCoverage>) {
...
fn branch_heavy_fixture(branches: u32) -> BranchFixture {
    ...
    for id in 0..branches {
        let line = id + 1;
        let line_start = byte_len(&source);
    ...
    ranges.insert(0, V8CoverageRange { start_offset: 0, end_offset: byte_len(&source), count: 1 });
...
fn bench_apply(c: &mut Criterion) {
    ...
    for (label, unicode) in RANGE_CASES {
        let (source, coverage, functions) = line_fixture(1_000, *unicode);
